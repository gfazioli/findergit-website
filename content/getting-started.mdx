...
FinderGit uses the `git` binary that ships with the Xcode Command Line Tools. On a brand-new Mac you may need to install them and accept the Xcode license — otherwise every Git action inside FinderGit fails, and the error it shows is git's own message saying the Xcode license hasn't been accepted.
...
Select a repository to reveal its detail — a nine-tab panel (Overview · Unstage · Commits · Branches · Release · Issues · Pull Requests · Maintenance · Trust) with the global Fetch / Pull / Push / Stash… / Refresh actions docked at its edge, reachable from any tab. See [Detail Panel](/docs/detail-panel). For a file you get its info and an inline diff; for a non-git file, basic info (size, date, type).

### Summary bar (bottom)
A live tally across the whole list — repository count, clean / dirty / ahead / behind (plus diverged, when any are), and total size.
...
- **If a row lights up under the pointer, it does something.** That is the rule across the whole app: lists whose rows are clickable highlight the one you are on, and the ones that are only there to be read do not. Where a row leads somewhere on the web, the cursor turns into a pointing hand.
...
- **[Detail Panel](/docs/detail-panel)** — what each of the nine tabs (Overview, Unstage, Commits, Branches, Release, Issues, Pull Requests, Maintenance, Trust) shows when a repo is selected.
